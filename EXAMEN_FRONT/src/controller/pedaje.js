...
const POSTExport = async (req = request, res = response) => {
  var { peajes, categorias, precio } = req.body;
...
const update = async (req=request, res=response) => {
  const { id } = req.params;
  const { peajes, categorias, precio } = req.body;
  try {
    const response = await axios.path(`https://api.ejemplo.com/items/${id}`, { peajes, categorias, precio });
